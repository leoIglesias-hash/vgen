...
| **E10** | VP9 −53,8 % **y** byte-idéntico entre corridas; H.264 daba bytes distintos por CPU (+22 / −74 B), curado en H-14b | H-9 / H-14 / H-14b | desde el 2026-09-04 **los dos carriles** cumplen el invariante 7: cuatro CPUs, un archivo |
...
| **E17** | **Hay una segunda clase de aparato, y sabe contar.** Smart TV Noblex, Android 11, **Chrome 142**: `quality si` (`getVideoPlaybackQuality` de verdad), `rvfc si`, cuota **2.637 MB**, `v0-vp9-alpha` **0/156 con deriva 0** y arranque 601 ms | reporte del Smart TV, 2026-09-04 (noche), tras H-22 | **es el aparato que puede arbitrar E15** (los tres planos): su contador no es el `webkitDroppedFrameCount` desacreditado de la caja. `rvfc` habilita sincronía de cuadro exacta **como mejora opcional por aparato, nunca requisito**. **La caja sigue siendo la clase principal**: el formato se diseña contra el piso |

| **E18** | **Solo el CRF de VP9 compra bytes; el GOP de 1 s cuesta más que el perfil; la cadencia variable no compra nada en este máster.** Matriz de 28 variantes (`tools/emit_matrix.py`, run `33936095399`, autocontrol: bitstream y píxeles idénticos a v0): VP9 crf 38 = 64 % con −0,005 SSIM; `cpu-used`, `tune-content screen`, `aq-mode 0` y alt-ref no cambian un byte (o 1,6 %); defaults de ffmpeg (High, GOP 250) = 43 % vs High+3B con GOP 15 = 53 %; 1 cuadro duplicado exacto en 231 | matriz H-6, 2026-09-05 | la receta es **por video** (regla 9), pero los ejes muertos no se vuelven a barrer; los 10 puntos del GOP de 1 s se pagan a propósito (S12); S6 refutada para este máster (P-004) |
| **E19** | **v1 existe y es determinista en la misma máquina:** VP9 crf 38 + Opus = 2.941.449 B (66,7 % de v0 **con audio**), H.264 High+3B crf 23 + AAC = 5.254.272 B (55 %), radio mp3 byte a byte, `dash-vp9/` por remux; dos pasadas idénticas | `emitir-v1`, run `33936096738`, 2026-09-05 | S13, S14 y S11 se miden con `72`/`74`/`75`; entre CPUs distintas el audio queda por ver (P-006) |
...
Orden (actualizado 2026-09-04, noche; ~~W-26b~~ cerrada en la caja esa misma noche): ~~H-14b~~ ~~H-12b~~ ~~H-16~~ ~~W-26b~~ ~~H-18~~ ~~H-18b~~ ~~H-20~~ ~~H-6~~ (ejecutada 2026-09-05, falta la foto) → ~~H-8a~~ (se sostiene en la caja, 2026-09-05) → ~~H-23~~ (aparcada 2026-09-06, sin reemplazo) → ~~P-008~~ (encoder portátil, ejecutada y medida 2026-09-06; **adoptada en B: el bundle manda**) → ~~P-008b~~ (cerrada 2026-09-06 noche: `portable` con dos runners de Windows + `pack-v1`, `v0/` con las huellas del bundle) → **H-26** (emisión desde la fuente, carril v2: 1280@20, color 709, VP9 dos pasadas, techo 20 MB; decisión del operador 2026-09-07 «el look fue un medio», v1 plan B) → E-E / E-F sobre v2 → **H-24** (incentivador armado en pausa) → radio con la primera tecla → **H-7** (borrador 0.1 escrito; **firma pendiente**) → **H-8** (muxer + archivo único); lo que falte de **H-10** corre aparte.
...
| **H-6 — matriz por bytes a igual look** ✅ 2026-09-05 ([`EMISION-V1.md`](EMISION-V1.md); falta la foto) | ¿cuántos bytes menos a igual gate? por eje: VP9 (CRF, `cpu-used`), **fps variable por segmento** (S6 → bytes, ya no fluidez), H.264 piso relajado (Main/High, `refs`, B dentro del GOP cerrado), zonas estáticas, paleta 4:2:0. Incluye una fila de **referencia** con los defaults de ffmpeg (el `producto.mp4`) bajo los mismos contadores | emisión **v1**: cada variante con sus segmentos (CMAF para H.264, WebM segmentado para VP9 → **S11**) | tabla en el REGISTRO con medición por aparato; **receta por perfil**, firmada por el operador |
| **H-14 — determinismo H.264** (**CERRADA: ejecutada en H-14b**) | (no es del aparato) | dos pasadas por corrida + `lscpu` | determinista por máquina, distinto por CPU, `cpu-independent=1` lo cura → adoptado en H-14b |
| **H-7 — spec `SPEC-VGEN.md`** (borrador 0.1, 2026-09-05 noche) | — | contenedor, manifiesto tabulado, guion, residencia, segmentos por rangos, mapa perfil → camino; sprites/cues/huecos/archivo único marcados ⏳ | aprobada por el operador; cada decisión trazable a una fila ([`SPEC-VGEN.md`](SPEC-VGEN.md) §11); lista de firma en §12 |
| **H-8a — el player del producto, como prototipo** (ejecutado 2026-09-05 noche; falta la foto) | ¿la forma entera del producto se sostiene en la caja desde la caché: anillo MSE sin atascos, publicidad ida/vuelta ≤ 1 s, incentivador ≤ 1 s, segunda apertura sin red? | `producto.html` + `GUION.tsv`; `ring()`; residencia H-15 en runtime; teclas de una cifra | foto de la caja y del Smart TV (SPEC §12); en la PC: 0 atascos, 48/186/163 ms |
| **H-23 — la imagen que gira encima del alfa** (ejecutada 2026-09-05; **aparcada 2026-09-06**: la caja la ve trabada con timer y con vsync; **no se reemplaza por un tercer video** — el tercer elemento es la ruleta, interactivo, y un video no responde) | ¿una imagen rotada por `drawImage` a 15 fps encima del incentivador con alfa le cuesta un cuadro a la caja? (pedido del operador: «esa prueba no la hicimos… siempre pensando en eficiencia») | `7` cicla en `producto.html`; `logo.png`; pintada cronometrada por carga | foto: `7` `7`, `4`, `9` (SPEC §12.6); en la PC: 0,20 ms med / 2,2 max por pintada |
...
| **W-26b — terminar la raíz** (**CERRADA 2026-09-04 noche en la caja**: *«sigue andando trabado pero sin pantallazos»*) | ¿la raíz con `?renderer=canvas2d` deja de dar pantallazo blanco? | auditoría servido-vs-repo escrita; las cuatro carpetas republicadas | sin pantallazos en la caja. Lo trabado es el **player 100 % JS**, ya medido en DIAG-003 (p50 233–290 ms, cuello en CPU): no es defecto de W-26 y es la razón por la que el producto va por `<video>` |
...
|---|---|---|
| **H.264 no determinista** (invariante 7) | **SALDADA 2026-09-04** (H-14b) | `cpu-independent=1` en la receta; cuatro CPUs dan el mismo archivo, los SHAs se citan sin salvedad |
