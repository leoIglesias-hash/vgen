...
| **H-14b** | `tools/emit_pieces.py` + workflow `emitir-v0` | **adoptar `cpu-independent=1`** (decisión del operador 2026-09-04, tras la explicación técnica: huellas estables para que la caché no se invalide por el runner). Acción: sumar `:cpu-independent=1` a `X264_BASELINE` y `X264_MAIN`; test que lo afirme; correr `emitir-v0` con `determinismo: true` **sin** `x264_extra` y verificar bytes idénticos entre pasadas; bajar el pack, publicar `v0/` con las huellas nuevas (copia previa en `deploy/`), fila en el REGISTRO con SHA-256 por pieza y la diferencia de bytes contra el pack del 09-01. Cierre: CI verde, dos pasadas idénticas en el log, huellas servidas = huellas del REGISTRO, invariante 7 marcado **cumplido** en EMISION §3.b |
