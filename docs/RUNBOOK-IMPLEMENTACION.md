...
[`PLAN-IMPLEMENTACION-ASCLH.md`](PLAN-IMPLEMENTACION-ASCLH.md)** (vigente desde el
...
([`PLAN-IMPLEMENTACION-ASCLH.md`](PLAN-IMPLEMENTACION-ASCLH.md) §3).
...
| **H-13** | `frontend/v0.html` (crece) + `tests/test_v0_page.js` | **por dónde entra el paquete** — con el pack ya publicado, **cero emisión nueva**. Teclas `9x` nuevas: **MSE H.264** (XHR de `dash/init.m4s` + `chunk-*.m4s` → `SourceBuffer('video/mp4; codecs="avc1.42E01F"')`, anexo encadenado por `updateend`; **S9**); **Blob concatenado** (`init + segmentos` en un solo Blob → `blob:`; **S10**); **intercambio de orden** (los mismos segmentos en otro orden, por MSE y por Blob; **S12**); **bucle de 60 s** (`ended` → `play()` y `loop`, contando `waiting`/`seeking` en cada vuelta); detección de `SourceBuffer.changeType`. Además: columna **«congelados»** (muestras de 100 ms sin avance de `currentTime`), «atascos» **descuenta el `waiting` inicial**, y la fila dice «contador ciego» cuando `total` no se movió. Ningún dígito suelto queda demorado (regla del mando) | filas en el REGISTRO por camino con las columnas de [`PLAN-IMPLEMENTACION-ASCLH.md`](PLAN-IMPLEMENTACION-ASCLH.md) §3.1; S9/S10/S12 marcadas en EMISION-V0 §4.c; gate ES5 verde; y **queda escrito qué camino implementa el muxer (H-8)** |
