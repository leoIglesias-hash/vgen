...
| **H-12** | `frontend/vgencache.js` (nuevo) + `frontend/v0.html` (crece) + `tests/test_vgencache.js` (nuevo) + `tests/test_v0_page.js` | **EJECUTADA hasta la pantalla 2026-09-02** (`0ce2cb4` → `9011fe7`, CI verde, publicada en `v0/`; REGISTRO «H-12 implementada hasta la pantalla»). Falta la foto de la caja: `84` + `95`, **reiniciar**, `95` + `85`. Era: **caché**: descarga por XHR con progreso → `Blob` → **IndexedDB** → reproducción desde `blob:`, con pineo por contenido (CACHE-001) y borrado de claves viejas | el pack sobrevive a un reinicio del aparato y reproduce sin red; degradación escrita si no persiste |
