...
| **W-26b** | `deploy/asciline-player/` + las cuatro carpetas de la raíz | **terminar W-26 como corresponde** (operador 2026-09-04: «dejemos eso cerrado»). Acción: bajar `index.html` y los `.js` servidos en `/`, `/1280-15/`, `/1280-12/`, `/1920-10/`; diff contra `frontend/live-player.html` + `playloop.js` + el resto; escribir en el REGISTRO qué cambia por carpeta (W-22..W-25 + W-26); republicar las keys que cambien con SHA verificado y token quemado; MANIFEST/README de deploy actualizados **antes** de subir. Cierre: las cuatro raíces sirven el `live-player.html` del repo, `?renderer=canvas2d` funciona en la caja (foto sin pantallazo blanco) |
