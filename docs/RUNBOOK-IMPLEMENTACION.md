...
| **H-10** | `frontend/v0.html` + `frontend/keypad.js` | **reproducirlo y que él nos diga.** La página **ya está escrita, publicada y verificada** en `https://iargen.com/player/v0/`: una sola pantalla sin scroll, un solo `<video>` para todas las piezas, mando numérico (`1` corre las 7 seguidas) y **tecla 95** para leer el reporte a pantalla completa. **Lo único que falta es abrirla en los aparatos** — eso lo hace el operador, nadie más | tabla de §5 de [`PLAN-DE-MEDICION.md`](PLAN-DE-MEDICION.md) llena para la TV box **y** 2-3 aparatos más; veredicto del operador textual; suposiciones **S1..S5, S7 y S8** marcadas sostenida/refutada en [`EMISION-V0.md`](EMISION-V0.md) §4 |
