...
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| **TV box** (la de DIAG-002/003) — **medida 2026-09-01** | Android 9, WebView **Chromium 70** (`TVBOX Build/PPR1.180610.011; wv`) | ✓ 0/156, 2.985 ms | ✓ 1/153, 2.813 ms | ✓ arranca en **931 ms**, deriva −1 ms; **contador ciego**; ojo: «perfecto, hasta más fluido» | ✓ **compone** («el verde alrededor, y dentro del círculo el video»); 1/155, 1.052 ms | ✓ **517 ms**, 2/155; Blob CMAF `init+16` = archivo, **1.286 ms** (H-13) | ✓ **probado (H-13)**: 2.033 ms por red, 156 cuadros, 0 atascos; `sequence` cose otro orden limpio; `changeType` sí | ✓ **persiste** al cierre de la app (2 piezas, 13,96 MB); cuota declarada **225 MB**; 25 MB entran, 50 MB de una vez cierra la app (defecto de la prueba, H-12b) | 0–2 de ~155 en 10 s donde el contador cuenta | ✗ | ✓ **encima**: 0/155 con canvas a 15 fps vs 1/156 sin (Baseline y VP9), deriva 0–1 ms | 1280×720 / 3840×2160 | **P0 y P1 confirmados** (clase principal por decisión del operador); **P2 confirmado** (persistencia, 2026-09-04) |
| _(celular del operador)_ | | | | | | | | | | | | | |
| **Smart TV Noblex** — **primer reporte 2026-09-04 (noche)**, tras H-22 | Android 11, WebView **Chrome 142** (`NoblexTV Build/RTO9.230812.001; wv`) | | | | ✓ **0/156, deriva 0, arranque 601 ms** | | ✓ declarado | ✓ declarado; cuota **2.637 MB** (12× la caja) | **el contador SIRVE: `quality si`** | **✓ sí** | | 962×541 dpr 1,33 / 3840×2160 | **otra clase** (Chrome moderno). Enriquece y arbitra; **no reemplaza** a la caja, que es la que consagra |
...
|---|---|---|---|---|
| **TV box** | **irregular**: 2.012 ms y 0 atascos la única vez que arrancó bien; después «se traba mucho al iniciar» (operador) | ✗ **inservible**: 14.223 ms, 2 atascos reales, deriva 95 ms | ✗ error de carga | ✓ 154 cuadros desde el init compartido; **por MSE** 156 cuadros, 0 atascos (H-13); concatenados en un Blob = archivo de 15,4 s; en otro orden por Blob **no** (123 cuadros, «se tilda»); bucle por `loop`: 3 `waiting` en 60 s |
...
290 ms/cuadro contra 66,7; el WebGL dibuja pero **no presenta**; y
`producto.mp4` (1280×720 @15, 4.130.240 B) *«reproduce muy bien»* por `<video>`
con decodificador de hardware.
