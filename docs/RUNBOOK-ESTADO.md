...
| H-12b | H | **cierre técnico de la caché**: techo en **tandas ≤ 5 MB** que nunca cierren la app, con la cuota declarada como primer techo reportado; `83` sola queda en play (gesto); caídos negativos en `blob:cmaf`; y en la caja: foto tras **apagar y prender** + `85` | **ejecutada hasta la pantalla 2026-09-04** (`432647b`, CI verde; publicadas `v0/index.html` y `v0/vgencache.js`, copia previa en `b53a739`). **Falta la foto de la caja**: techo real con `84`, `85` tras apagar y prender, y `83` sola mostrando video | no |
| H-16 | H | **Hobo por defecto + reforma de la página de pruebas**: `HoboStd.ttf` servida desde `v0/` (`@font-face`, espera por `measureText`, cae a `monospace`), fuente por defecto de la capa; el `1` corre solo lo **no consagrado**; teclas ya probadas fuera de la leyenda → `docs/MANUAL-TECLAS-V0.md`; leyenda ≥ 10 teclas a la izquierda, tabla de filas más alta | **ejecutada hasta la pantalla 2026-09-04** (`bbb92a1`+`52f8927`+`c4bb8ee`, CI verde; publicadas `v0/HoboStd.ttf` y `v0/index.html`, bucket 62 -> 63 keys). Verificado en el navegador contra lo servido: fuente detectada en 165 ms, 12 teclas -13 desde H-18- sin cortes a 1280 y a 420, sin solape con el video, `83` inmediata arranca VP9. **Falta la foto de la caja** | no |
| W-26b | — | **terminar W-26 como corresponde** (operador 2026-09-04): auditar `index.html` servido en `/`, `/1280-15/`, `/1280-12/`, `/1920-10/` contra `frontend/live-player.html` + JS del repo, listar qué cambia, republicar con copia previa en `deploy/` | **ejecutada 2026-09-04**: auditoría previa (64 rutas: 56 iguales, 8 distintas -la misma página en 4 carpetas, solo le faltaba W-26-), 8 keys republicadas y verificadas, `playloop.js` anotado en el manifiesto (estaba servido y sin registrar), re-auditoría **64/64 iguales**. Medido: sin el parámetro la raíz pasa **240 ms en WebGL** al abrir; con `?renderer=canvas2d`, nunca. **Falta la foto de la caja sin pantallazo blanco** | no |
| H-18 | H | **dos `<video>` a la vez**: loop (VP9) + efecto con alfa (`v0-vp9-alpha`) encima, mismo contador; responde «¿los efectos pueden ser video?» (pregunta del operador 2026-09-04) y el techo de planos simultáneos (DISENO §10) | **ejecutada hasta la pantalla 2026-09-04** (`c1648b0`..`a564090`, CI verde; `v0/index.html` republicada). Tecla `87`. Medido en la PC: VP9 solo **0/156** caídos; con el segundo plano **5..12/157** abajo y **4..8/153** arriba → **los dos se sostienen y los dos cuentan, pero el segundo cuesta**. La PC refuta, la caja consagra. **Falta la foto de la caja** | no |
