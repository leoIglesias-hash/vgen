...
| H-9 | H | **pack v0 — el primer video, por suposición** (`tools/emit_pieces.py` + workflow `emitir-v0`): Baseline, Main, VP9, VP9+alfa y `MANIFEST.tsv` desde el máster, con parámetros explícitos y hash por pieza | **en curso** — emisor y workflow escritos; falta la corrida y su fila en el REGISTRO | sí (piezas nuevas) |
