...
> ## ▶ PRÓXIMA ACCIÓN: **RECETA v2 FIRMADA 2026-09-08 (noche): `--fps 15 --ancho 1280 --vp9-crf 18 --h264-crf 14 --techo 20000000`** — operador, plano `3` de `player/v1/`: *«de momento me quedo con la 3, pero está bien tener la opción para el upgrade a la 2 de fps… pero la 3 en este tipo de videos se ve perfecto»*. Upgrade declarado: `--fps 20`. Firmada en los tres lugares (`armar.RECETA_V2`, `emitir.ps1 -RecetaV2`, EMISION-V2 §2; `test_portable_bundle` los cruza). **Lo que sigue, en orden:** (1) **emisión completa con la receta firmada** desde la PC del operador (`cd vgen-portable; .\emitir.cmd -Fuente "…TKN-2443-GANADOR- 15seg-.mp4" -Out outputs\v2-firmada`, sin `-Receta`; el bundle ya tiene el `emit_v2.py` nuevo copiado a mano — o bajar el bundle de la próxima corrida `portable`): VP9 debería repetir `1c9b383c93e5…`, y trae el **H.264 crf 14 @15** que todavía no existe; (2) publicarla en `v0/` como pack v2 definitivo (ritual, deploy antes; retirar `v2-vp9-crf10` y `dash-v2-vp9-crf10/`; `MANIFEST-v2.tsv` nuevo); (3) **`producto.html` + `GUION.tsv` a v2** (anillo MSE desde `dash-v2-vp9/` @15, H.264 piso, radio `v2-ambiente.mp4`) y foto de la caja con el producto entero; (4) **carril v2 en `portable`** (inputs `fuente_url`/`fuente_sha256`, dos runners, `pack-v2`, ffprobe de etiquetas) reproduciendo lo de la PC — P-010 (Opus no cpu-independent) sigue abierta y decide el operador; (5) papel: SPEC §9, DISENO-CALIDAD §5.3 cerrado, H-26 y H-27 cerradas. Después: E-E/E-F sobre v2, H-24 y radio (pausa), H-7, H-8. Pendiente menor: repetir `78` en `v0/` para confirmar que lo «trabado» era esa página. **Antes: H-27 (2026-09-08 noche): la caja vio v2 (crf 10 y 18) MEJOR que v1 pero TRABADO** — el operador se concentra en **crf 18** y pide 15 fps y 512 colores; sumo `--sin-altref`. `emit_v2.py` gana `--colores N` (`tools/cuantizar_y4m.py`), `--sin-altref`, `--solo-vp9`; **página nueva `frontend/v1.html` → `player/v1/`** con `PLANES.tsv`: un plano por tecla (`1` v1-256, `2` v2 crf18@20, `3` @15, `4` 512 col, `5` @15+512, `6` sin alt-ref, `7` H.264), misma tecla pausa/sigue, **`0` = pantalla entera**; `ir.html` tecla `8` (el operador lo re-sube). **HECHO 2026-09-08 (noche, operador: «ejecutá esto vos»): los cuatro planos emitidos desde esta sesión en su PC (6 min, código 0) y `v1/` PUBLICADA** (`aea30ff` deploy antes; 7 keys, las 7 bajadas y SHA-idénticas, token quemado, 403 comprobado). Bytes: `3` 15 fps 6.011.270 (−14 %, SSIM 0,9916); `6` sin alt-ref 6.775.111 (−3 %, 0,9902); **`4` 512 colores @20 10.376.435 y `5` @15 8.931.539: a igual crf la paleta pesa +49 % y pierde 0,016 de SSIM** (escalones sin dither = bordes = lo caro; v1 era chico por el crf 38, no por los 256 colores). Manifiestos en `docs/matrices/2026-09-08-H27-*`. **VEREDICTO 2026-09-08 (noche), textual:** *«tanto 2 y 3 se ven bárbaro. controlar los colores no nos dio ganancia para nada. pero los fps sí pueden darnos menos pérdida. así que es algo a tener en cuenta»*. El mismo crf 18 @20 (`0f4683bc…`) que en `v0/` `78` se veía trabado es fluido en `v1/`: «trabado» fue de la página v0, no del archivo (repetir `78` en v0 para confirmar). **Paleta retirada de la receta.** **Candidatas aprobadas a ojo: crf 18 @20 o crf 18 @15**; el operador elige entre las dos y se firma en los tres lugares (`armar.RECETA_V2`, `emitir.ps1`, EMISION-V2 §2), `producto.html` pasa a v2, después el carril CI. `6`/`7` sin veredicto, no urge. La receta v2 no se firma hasta eso; `78`/`79` de v0 quedan. **Antes: H-26 en curso — PRIMERA EMISIÓN v2 HECHA el 2026-09-08 desde la PC del operador** (Intel i7-13700H, bundle `17d9903`): fuente `6e78efa38e10…` **etiquetada 709 tv** (supuesto verificado), base 1280×720 @20 = 308 cuadros, `v2-vp9` 3.757.946 B ssim 0,987 `74ea7f4b…`, `v2-h264` 6.600.291 B ssim 0,990 `f75a2125…`, radio m4a 614.020 B, DASH 16 seg. **Matriz v2 HECHA el 2026-09-08 (615 s en la PC):** VP9 no toca el techo ni a crf 10 (10.825.432 B, ssim 0,9938); H.264 cruza entre crf 14 (14.399.566 B, pasa) y 11 (21,9 MB, SUPERA). **Candidato por la regla: VP9 crf 10 + H.264 crf 14**; alternativa de ahorro VP9 crf 18 (6,9 MB). **El operador emitió LAS DOS (crf 10 y crf 18, H.264 crf 14 byte-idéntico) y están PUBLICADAS en `v0/`** (2026-09-08 tarde: 42 keys, SHA verificados, ritual completo; `v2-ambiente` servido como `.mp4` porque el Worker no conoce `.m4a`). **Tecla `78` = v1 / v2 crf 10 / v2 crf 18 a ojo** (enteros, en bucle, cada `78` pasa a la siguiente) y **`79` = lote v2 medido** (gate caídos ≤ 3 %). **FALTA LA FOTO DE LA CAJA**: `78` × 3 y el veredicto a ojo (¿se distingue crf 10 de crf 18?), `79` + `95`. Con eso se firma la receta en los tres lugares y después el carril v2 en `portable`. Registro: REGISTRO 2026-09-08 (tres entradas); archivo `docs/matrices/2026-09-08-MATRIZ-v2-pc-operador.tsv`. Emisor listo con CI verde: fuente → 1280@20, 709 explícito, VP9 dos pasadas + alt-ref, H.264 High, radio de la fuente, `dash-v2-vp9/`, techo 20 MB (código 4 = emitido, no se publica), `--barrer` = matriz v2 contra la fuente. Directiva del operador: *«debe correr lo mismo de mi PC en CI; trabajaremos sobre eso cuando terminemos las optimizaciones desde la PC»* → carril v2 en `portable` DESPUÉS. Luego: publicar en `v0/` con tecla v1/v2 y foto de la caja. **H-24 y la radio en pausa.** Sigue pendiente: comparar el SHA de `emitir.cmd` v1 con la corrida 34077462713 (P-008b). **Bundle con `emit_v2.py`: artifact `vgen-portable` de la corrida 34083813584** (zip `a3d31b3f7250…`); esa corrida salió ROJA en la comparación porque el runner 2 fue Intel y **el Opus no es cpu-independent** (VP9/H.264/DASH idénticos) → **P-010** abierta (Opus emitido una vez y muxeado por copia), decide el operador.
>
...
| 2026-09-06 (noche) | **P-008b CERRADA — «el bundle manda» en pie** (`706f21f`→`c642a0c` código, regression verde en `c642a0c` tras dos correcciones del test nuevo; `1db7eaa`+`1161928` papel y `deploy/`). Workflow `portable`: pasada 1 (`armar`) + pasada 2 (`reproducir`, otro runner, zip publicado, sin checkout), `comparar` exige las cuatro filas del manifiesto idénticas y publica `pack-v1`; Linux `continue-on-error`, informativo. Corrida **34077462713**: cuatro piezas idénticas (VP9 `4b0714ed21ca`, H.264 `175722d34d0f`, mp3 `c886263508da`, DASH `7c89d04d9301`), 107/117 s, zip 166.236.545 B `75a0c3c8d28a`. **`v0/` republicada** (21 keys, verificadas las 22, token quemado). Papel: EMISION-V1 §3, SPEC §9, invariante 8, ENCODER-PORTATIL §6/§8, LEEME. Queda: el operador baja el bundle y compara su SHA. REGISTRO «P-008b CERRADA» |
| 2026-09-08 (noche) | **H-27: la caja vio v2 (crf 10 y 18) MEJOR que v1 pero TRABADO** (operador: *«se ven mejor definitivamente que la de 256 colores.. pero el problema es que se traban»*). Se sigue con **crf 18** y se buscan planos de fluidez a ojo, uno por vez: 15 fps y 512 colores (pedidos), sin alt-ref (propio: v1 nunca tuvo cuadros ocultos) y el H.264 de testigo. `emit_v2.py` gana `--colores N` (`tools/cuantizar_y4m.py`, paleta adaptativa Oklab sobre la referencia), `--sin-altref` y `--solo-vp9`. **Página nueva `frontend/v1.html` → `player/v1/`** (pedido: *«sino apretás una tecla y se hace una ensalada bárbara»*): `PLANES.tsv`, un plano por tecla, misma tecla pausa/sigue, **`0` = pantalla entera pone/saca**, nada se corre solo; `ir.html` tecla `8`. H-26 sigue abierta: la receta no se firma hasta que la caja diga cuál es fluido | La emisión la hace el operador desde `vgen-portable` (los dos archivos nuevos se copian a mano al bundle de `17d9903`); el CI no reproduce todavía (directiva vigente). Registro: REGISTRO 2026-09-08 (noche); `EMISION-V2.md` §7; `MANUAL-TECLAS-V0.md` sección `v1/` |
| 2026-09-08 (noche, 2) | **H-27: los cuatro planos emitidos desde esta sesión en la PC del operador («ejecutá esto vos») y `v1/` PUBLICADA** (7 keys, SHA verificados). Hallazgo: **a igual crf la paleta de 512 colores pesa +49 % y pierde 0,016 de SSIM** (los escalones sin dither son bordes, lo caro para VP9); 15 fps −14 %, sin alt-ref −3 %. v1 era chico por el crf 38, no por los 256 colores | El contador refuta la hipótesis de bytes de la paleta antes de la caja; los planos van igual porque el que firma es el ojo. Falta la foto plano por plano. REGISTRO 2026-09-08 (noche) segunda entrada |
| 2026-09-08 (noche, 3) | **Veredicto de la caja en `v1/`**, textual: *«tanto 2 y 3 se ven bárbaro. controlar los colores no nos dio ganancia para nada. pero los fps sí pueden darnos menos pérdida. así que es algo a tener en cuenta»*. El crf 18 @20 que en v0 `78` se veía trabado es fluido en `v1/`: la observación era de la página, no del archivo. **Paleta retirada de la receta v2**; candidatas aprobadas a ojo crf 18 @20 y @15, elige el operador | Los planos `6`/`7` quedan sin veredicto y no se emite nada nuevo. Pendiente: repetir `78` en v0 para confirmar que lo trabado era la página; elegir cadencia; firmar en los tres lugares; `producto.html` a v2; carril CI. REGISTRO 2026-09-08 (noche) tercera entrada |
| 2026-09-08 (noche, 4) | **RECETA v2 FIRMADA: `--fps 15 --ancho 1280 --vp9-crf 18 --h264-crf 14 --techo 20000000`** (operador: *«de momento me quedo con la 3, pero está bien tener la opción para el upgrade a la 2 de fps… la 3 en este tipo de videos se ve perfecto»*). Upgrade declarado `--fps 20`. En los tres lugares, test cruzado | Falta la emisión completa con la receta (H.264 @15), publicarla como pack v2 definitivo, `producto.html` a v2, carril CI. REGISTRO 2026-09-08 (noche) cuarta entrada |
