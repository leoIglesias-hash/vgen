...
## Próxima acción (actualizado 2026-08-31, post-cierre de F6/S-4)
...
1. **Del operador:** probar `https://iargen.com/player/` en el **celular y el Smart TV**
   (antesala de F8). Ya aprobó el v3 a ojo en desktop («se ve igual», Instancia 029).
   Puede comparar v2 vs v3 en el mismo dominio: raíz = producto 1280@15 **v3**;
   variantes `/player/1280-15/` (v2), `/player/1280-12/`, `/player/1920-10/`.
2. **Siguiente fase: F8 (S-6)** — validación física en TV (diagnostic-player, p95,
   MEM-001, go/no-go de v2/v3); F8-2 debe incluir el **1920** (directiva del operador:
   el front procesa cualquier resolución/fps; el 1920 volverá a probarse a más fps).
   **INT-005 sigue CONDICIONADO** a que los gates físicos de F8 fallen para el overlay
   nativo (dirección del operador 2026-08-30). Tareas en el
   [`RUNBOOK-IMPLEMENTACION.md`](RUNBOOK-IMPLEMENTACION.md) §3.
3. Opcionales sin fase: E-11 (flags de audio), W-15 (camino ASCII). Menor: si se
   retoma el 960, re-medirlo con refit 5. Mejora anotada de infra: si el workflow
   `publish-player` se vuelve habitual, migrar los pins a un `__pins.json` en el bucket.

**Estado de fases: F0-F7 completas y verificadas; S-1..S-5, S-7 cerradas; solo queda
S-6 (F8) + opcionales.** El detalle de cada cierre: tabla de fases abajo,
[`ejecutados/`](ejecutados/README.md) y las tablas archivadas (ver «Tareas cerradas»).
