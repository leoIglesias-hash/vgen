...
3. **F11 (S-10)** — formato v4: E-30 (LOD horneado, **sin** cambio de formato, medible
   solo) → F11-1 (opcode `LOD2`) → F11-2 (**transparencia**, pedido del operador) →
   F11-3 (espejo JS + cross-test) → F11-4 (barrido y adopción).
...
| E-30 | F11 | `--lod-tile`: horneado 2×2 en tiles de bajo detalle (**sin** cambio de formato) | pendiente | sí |
