...
3. **H-11 EJECUTADA hasta la pantalla** (`fd9a7ab`, CI verde; REGISTRO «H-11
   ...
   rect 15 fps / pantalla una vez) sobre Baseline y VP9, teclas `930`..`933`,
   contador `oculto` (Chromium pausa el video mudo con la página oculta).
   **Falta solo la foto de la caja:** el operador corre **`930`** (≈ 2 min,
   incluye `blob:` + `blob concat` seguidos), `95` y foto; `933` a ojo mientras
...
| H-11 | H | **la bifurcación de layout**: canvas de intervención **encima** del `<video>`, con tres cargas, sobre Baseline y VP9, contra la línea de base medida (suposición S5) | **EJECUTADA hasta la pantalla** (`fd9a7ab`, CI verde, publicada en `v0/`); cierra con la foto de la caja (`930` + `95`) | no |
