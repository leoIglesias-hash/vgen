...
> ## ▶ Próxima acción: **RECETA v2 FIRMADA 2026-09-08 (noche): `--fps 15 --ancho 1280 --vp9-crf 18 --h264-crf 14 --techo 20000000`** (operador, plano `3` de `v1/`: *«de momento me quedo con la 3, pero está bien tener la opción para el upgrade a la 2 de fps… la 3 en este tipo de videos se ve perfecto»*; upgrade declarado `--fps 20`). Firmada en los tres lugares (`armar.RECETA_V2`, `emitir.ps1 -RecetaV2`, EMISION-V2 §2; el test los cruza). **Sigue, en orden:** (1) emisión completa con la receta firmada desde la PC (`emitir.cmd -Fuente … -Out outputs\v2-firmada`, sin `-Receta`): trae el H.264 crf 14 @15 que falta; (2) publicarla en `v0/` como pack v2 definitivo (ritual; deploy antes; retirar crf 10 y su DASH); (3) `producto.html` + `GUION.tsv` a v2 (anillo MSE desde `dash-v2-vp9/`, radio `v2-ambiente.mp4`); (4) carril v2 en `portable` reproduciendo lo de la PC (P-010 Opus sigue abierta); (5) SPEC §9, DISENO-CALIDAD §5.3 cerrado. Después: E-E/E-F sobre v2, H-24 y radio (en pausa), H-7, H-8. **Antes: H-27 (2026-09-08 noche) — la caja vio v2 (crf 10 y 18) MEJOR que v1 pero TRABADO** (*«se ven mejor definitivamente que la de 256 colores.. pero el problema es que se traban»*). Se concentra en **crf 18** y se buscan cuatro planos de fluidez, emitidos por el operador desde `vgen-portable` con `emit_v2.py` nuevo: `--fps 15`, `--colores 512` (paleta adaptativa Oklab, `tools/cuantizar_y4m.py`), los dos juntos, y `--sin-altref` (VP9 sin cuadros ocultos, como v1; sospecha propia). **Página nueva `frontend/v1.html` → `iargen.com/player/v1/`** (pedido: *«sino apretás una tecla y se hace una ensalada bárbara»*): un plano por tecla (`1` v1-256, `2` v2 crf18@20, `3` @15, `4` 512 col, `5` @15+512, `6` sin alt-ref, `7` H.264), misma tecla pausa/sigue, **`0` = pantalla entera pone/saca**; `ir.html` tecla **`8`** (el operador vuelve a subirlo). Planos en `frontend/v1-planes.tsv` → `v1/PLANES.tsv`. **HECHO 2026-09-08 (noche, «ejecutá esto vos»): los cuatro planos emitidos desde esta sesión en la PC del operador y `v1/` PUBLICADA (7 keys, SHA verificados).** Bytes: 15 fps 6,0 MB (−14 %, SSIM igual); sin alt-ref 6,8 MB (−3 %); **512 colores 10,4 MB @20 y 8,9 MB @15: a igual crf la paleta cuesta +49 % de bytes y −0,016 de SSIM** (los escalones sin dither son bordes, y el borde es lo caro; lo que hacía chico a v1 era el crf 38, no los 256 colores). **VEREDICTO DE LA CAJA (2026-09-08 noche, en `v1/`):** *«tanto 2 y 3 se ven bárbaro. controlar los colores no nos dio ganancia para nada. pero los fps sí pueden darnos menos pérdida»*. El mismo archivo crf 18 @20 que en `v0/` `78` se veía trabado, en `v1/` (un video por vez) es fluido: «trabado» fue una observación de la página v0, no del plano (pendiente: repetir `78` en v0 para confirmarlo). **La paleta se retira de la receta** (queda como herramienta). **Candidatas aprobadas a ojo: crf 18 @20 (7,0 MB) o crf 18 @15 (6,0 MB)**; elige el operador; `6`/`7` sin veredicto, no urge. Después: firmar en los tres lugares, `producto.html` a v2, carril CI. **Antes: H-26 en curso — primera emisión v2 HECHA el 2026-09-08 desde la PC del operador** (Intel i7-13700H; fuente `6e78efa38e10…` etiquetada 709 tv, 308 cuadros @20; VP9 crf 34 = 3,76 MB ssim 0,987, H.264 crf 21 = 6,6 MB ssim 0,990: **el techo de 20 MB queda 5× arriba**). **Matriz v2 HECHA** (615 s): VP9 no toca el techo ni a crf 10 (10,8 MB, ssim 0,9938); H.264 cruza entre crf 14 (14,4 MB) y 11 (21,9 MB). **Las dos recetas (VP9 crf 10 y crf 18, H.264 crf 14) emitidas por el operador y PUBLICADAS en `v0/`** (2026-09-08 tarde, 42 keys, SHA verificados). **Tecla `78`** = v1 / v2 crf 10 / v2 crf 18 a ojo (enteros, en bucle); **`79`** = lote v2 medido. **FALTA LA FOTO DE LA CAJA** (veredicto a ojo + caídos ≤ 3 %); con eso se firma la receta en los tres lugares y después el carril v2 en `portable` ([`docs/EMISION-V2.md`](docs/EMISION-V2.md) §4-6). El emisor `tools/emit_v2.py` (fuente → 1280@20, color 709 explícito, VP9 dos pasadas + alt-ref, H.264 High, radio de la fuente, DASH, **techo 20 MB por pieza**, `--barrer` = matriz v2 contra la fuente) está escrito y con CI verde. Directiva del operador 2026-09-07: *«debe correr lo mismo de mi PC en CI; trabajaremos sobre eso cuando terminemos las optimizaciones desde la PC»* → el carril v2 del workflow `portable` se cablea DESPUÉS de elegir la receta. Luego: publicar v2 en `v0/` con tecla v1/v2 y foto de la caja. v1 (256 colores) = plan B. **H-24 y la radio siguen en pausa.** Pendiente del operador también: comparar el SHA de su `emitir.cmd` v1 con la corrida 34077462713 (P-008b). **Bundle con `emit_v2.py`: corrida 34083813584** (salió roja en la comparación: runner Intel → **el Opus no es cpu-independent**, VP9/H.264/DASH sí; **P-010** abierta, decide el operador).
>
