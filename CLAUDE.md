...
> ## ▶ Próxima acción: **H-26 — emisión desde la fuente (carril v2)**, decisión del operador 2026-09-07: *«el look fue un medio»*. El video sale del clip original a **1280@20**, color 709 explícito, VP9 dos pasadas + alt-ref, **techo 20 MB por pieza**; el máster indexado (v1) queda como plan B si el hardware se pone lento. Leer [`docs/DISENO-CALIDAD-VGEN.md`](docs/DISENO-CALIDAD-VGEN.md) §5.3 y la fila H-26 de [`docs/RUNBOOK-IMPLEMENTACION.md`](docs/RUNBOOK-IMPLEMENTACION.md). Pregunta chica pendiente del operador: dónde vive la fuente para el CI (bucket pineado o solo bundle local); la tarea arranca por el emisor y sus tests. **H-24 y la radio siguen en pausa.** El operador además baja `vgen-portable` de la corrida 34077462713 y compara el SHA de su `emitir.cmd` (P-008b).
>
